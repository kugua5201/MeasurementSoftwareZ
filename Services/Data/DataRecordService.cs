...
ORDER BY MeasurementTime DESC;";
                    ...
                    for (var i = 0; i < maxChannelCount; i++)
                    {
                        if (i < channels.Count)
                        {
                            ...
                            row.AddRange([
                                ...
                                QuoteCsv(channel.MeasuredValue.ToString(CultureInfo.InvariantCulture)),
        ...
        private static IEnumerable<CsvColumnDefinition> GetDefaultColumns()
        {
            return [
                ...
                new CsvColumnDefinition("MeasuredValue", "测量值", CsvColumnScope.Channel, (_, channel, _) => channel?.MeasuredValue.ToString(CultureInfo.InvariantCulture) ?? string.Empty),
